function convert_gtruth_YOLO_v2_videolabeler(gname,folder,nc)
...
for ll = 2:length(indices) %ii=1:size(gTruth.ROILabelData.video_LEFT_2022_08_27_17_28_01_313_Trim,1)
    ii = indices(ll);
%     %Video files
...
%     imwrite(video_frame,strcat(path_to_images,'/', num2str(ll),'.png'));
    ...
    for jj=1:(length(label_definitions)-3) %Go throuh the different labels
        for kk=1:size(labels{ii,jj+1},1)
            %coordinates = labels{ii,jj+1}(kk,1).Position;
    ...
    if ~isempty(temp_labels)
        ...
        %fileID = fopen(strcat(path_to_labels,'/', gTruth.DataSource.SignalName+'_'+num2str(fnum),'.txt'),'w');
        fileID = fopen(strcat(path_to_labels,'/', num2str(fnum),'.txt'),'w');
        fprintf(fileID,'%u %12.8f %12.8f %12.8f %12.8f\n',temp_labels');
        fclose(fileID);
        video_frame = read(v,ii);
        %imwrite(video_frame,strcat(path_to_images,'/', gTruth.DataSource.SignalName+'_'+num2str(fnum),'.png'));
...
%saves the image count to a file
nc = fnum
cd ..
nc = nc - 1
save('nc.mat','nc')
